...
router.post("/", (req, res) => {
  ...
  ReviewRecipe.createReviewRecipe(
    detailReview,
    rateReview,
    recipeId,
    (err, newReviewRecipe) => {
...
router.get("/recipe/:id", (req, res) => {
  ...
  ReviewRecipe.getReviewsByRecipeId(recipeId, (err, reviews) => {
    ...
    if (!reviews || reviews.length === 0) {
      return res
        .status(406)
        .json({ error: "Reviews not found for this recipe" });
    }
    res.json(reviews);
  });
});
...
router.put("/:id", (req, res) => {
  ...
  ReviewRecipe.updateReviewRecipe(
    reviewId,
    detailReview,
    rateReview,
    recipeId,
    (err, updatedReviewRecipe) => {
      if (err) {
        return res.status(500).json({ error: err.message });
      }
      if (!updatedReviewRecipe) {
        return res
          .status(406)
          .json({ error: "Review recipe not found or not updated" });
      }
...
router.delete("/:id", (req, res) => {
  ...
  ReviewRecipe.deleteReviewRecipe(reviewId, (err, deleted) => {
    ...
    if (!deleted) {
      return res
        .status(406)
        .json({ error: "Review recipe not found or not deleted" });
    }
